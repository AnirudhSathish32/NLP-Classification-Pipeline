...
def _load_bert_model():
    """
    Load the BERT checkpoint and tokenizer from disk.
...
def _run_bert(text: str) -> ModelResult:
    ...
    What it does:
        Tokenises the raw text inline using the loaded BertTokenizerFast,
        runs a forward pass through BertForSequenceClassification,
        applies softmax to the logits, and returns the predicted class
